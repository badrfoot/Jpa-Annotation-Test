...
import org.badr.orm.jpa.model.inheritance.MyEmployee;
import org.badr.orm.jpa.model.inheritance.Person;
...
//				   (imageDB.getComputerLocation().size()>0) );
	...
	public void shouldSortCollection() {
		...
		image2.setName("MINE");
		computer1.setIpAddress("10.10.10.10");
		computer1.setPcModel("HP");
		computer2.setIpAddress("20.20.20.20");
		computer2.setPcModel("DELL");
		computer3.setIpAddress("15.15.15.20");
		computer3.setPcModel("Toshiba");
		computer4.setIpAddress("15.15.15.20");
		computer4.setPcModel("ANA");

		List<String> locations = Arrays.asList("HardDisk", "USB");
		image2.setLocations(locations);

		SortedSet<Computer> computers = new TreeSet<>();
//		List<Computer> computers = new ArrayList<>();
		...
		transaction.begin();
		Image image3 = entityManager.find(Image.class, image2.getId());
		
		transaction.commit();
		
		System.out.println("*********************************");
		image3.getComputers().stream().forEach(c -> System.out.println(c.getIpAddress()));
